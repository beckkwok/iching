...
- **Rive**: `rive ^0.14.11` (`rive_native`) drives the bottom-nav icons on Android/iOS. It **crashes on Windows** (access violation), so `HomeShell.enableRiveAnimations` defaults to off on Windows and the nav falls back to Material icons. (rive 0.13's `rive_common` avoided the crash but hardcoded the ClangCL VS toolset.)
